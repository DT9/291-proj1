...
def checkDataType(vN):
    ...
    if(vN == "health_care_no" or vN == "employee_no" or vN == "type_id" or vN == "test_id" or vN == "patient_no" ):
        dataType = "int"
    if(vN == "lab_name" or vN == "medical_lab"):
        dataType = "varchar(25)"
    if(vN == "test_name"):
        dataType = "varchar(48)"
    if(vN == "name" or vN == "clinic_address" or vN == "address"):
...
def checkValid(variableName, inputValue):
    if(inputValue == ""):
        print("Empty string.")
        return False
    if((inputValue == "null" or inputValue == "NULL") and (variableName == "name" or variableName == "clinic_address" or variableName == "address" or variableName == "phone" or variableName == "test_name")):
    ...
    while(True):
        ...
        while(True):
            ...
            if(response == "y" or response == "Y"):
                dataType = checkDataType(variableName)
                if((("date" == dataType) or ("char" in dataType)) and not (inputValue == "null" or inputValue == "NULL")):
